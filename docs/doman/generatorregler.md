...
*Läsanvisning, ingen regel (2026-09-23):* avsnittet *Yta per spelare* i `passuppbyggnad.md` innehåller två **granskningskriterier** för övningsbanken: ett golv för yta per spelare och ett minsta längdmått för övningar i `fas-13-14` och `fas-15-19` som ska öva djupled. Båda används när en övning skrivs och granskas, inte av generatorn. Generatorn väljer aldrig bort ett moment för att ytan är trång, rymlig eller kort; den kontrollerar bara att momentet får plats på den yta ledaren har valt (R-092). Varken kvadratmetertalen eller längdmåtten ska alltså implementeras som filter. Om något av dem någon gång ska bli en regel är det ett eget beslut av användaren och en egen ändring av den här filen, med ett nytt regel-ID.
